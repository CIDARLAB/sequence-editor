...
public class Annealer {
    ...
    public String anneal(RichSequence fwdSeq, RichSequence revSeq) {
        ...
        try {
            revSeq3to5Prime = DNATools.complement(revSeq);
        } catch (Exception ex) {
    ...
    protected String generateAnnealingString(Alignment alignment) {
        ...
        try {
            ...
            if (forwardSymbolList.length() == reverseSymbolList.length()) {
                ...
                annealingString = alignment.symbolListForLabel(alignment.getLabels().get(0)).seqString() + matchCharString + alignment.symbolListForLabel(alignment.getLabels().get(1)).seqString();
