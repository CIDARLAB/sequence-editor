...
public class NeedlemanWunsch extends SequenceAlignment {
  ...
  public int pairwiseAlignment(SymbolList query, SymbolList subject)
    ...
    Sequence reverseComplementssubject=null;
        try {
        String reverseSubjectSeq=new StringBuffer(subject.seqString()).reverse().toString();

        SymbolList revSubject = DNATools.createDNA(reverseSubjectSeq);

        SimpleRichSequence reversedSubjectSequence = new SimpleRichSequence(new SimpleNamespace("org.biofab"), "", "",1, revSubject, 1.0);
         reverseComplementssubject=new RevCompSequence(reversedSubjectSequence);
        } catch (Exception e) {
          ...
          for (i = this.CostMatrix.length - 1; i > 0;) {
            do {
              ...
              if (i == 0) {
                ...
                if (squery.symbolAt(i) == reverseComplementssubject.symbolAt(j))
                  path = '|' + path;
                else
                  path = ' ' + path;
                align[0] = st.tokenizeSymbol(squery.symbolAt(i--)) + align[0];
